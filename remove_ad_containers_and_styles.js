/**
 * posts 디렉토리 내의 모든 HTML 파일에서 광고 컨테이너와 관련 CSS 스타일을 제거하는 스크립트
...
const adDivPattern = /<div class="post-ad">[\s\S]*?<\/div>/g;

// 광고 관련 CSS 스타일 패턴
const adStylePattern = /\.post-ad\s*\{[\s\S]*?\}/g;
...
files.forEach(file => {
    ...
    try {
        ...
        // 광고 컨테이너 또는 스타일이 있는지 확인
        const hasMiddleAd = middleAdPattern.test(content);
        const hasBottomAd = bottomAdPattern.test(content);
        const hasAdDiv = adDivPattern.test(content);
        const hasAdStyle = adStylePattern.test(content);
        
        // 광고 컨테이너와 스타일 제거
        if (hasMiddleAd || hasBottomAd || hasAdDiv || hasAdStyle) {
            ...
            // 기타 광고 div 제거
            content = content.replace(adDivPattern, '');
            
            // 광고 관련 CSS 스타일 제거
            content = content.replace(adStylePattern, '');
            
            // 변경된 내용 저장
            fs.writeFileSync(filePath, content, 'utf8');
            
            modifiedCount++;
            console.log(`✅ ${file} - 광고 컨테이너 및 스타일 제거 완료`);
        } else {
            console.log(`ℹ️ ${file} - 광고 관련 요소 없음`);
        }
...
console.log(`\n처리 완료: 총 ${processedCount}개 파일 중 ${modifiedCount}개 파일에서 광고 관련 요소 제거됨`); 